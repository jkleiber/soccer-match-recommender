...
BLOWOUT_FACTOR = 1.6 #Experimentally determined by plugging A^(goal difference) into google calc until it looked right
IMPORTANCE_CONSTANT = 1.5
UPSET_GD_MULTIPLIER = 1.75
UPSET_VALUE_EXP = 1.35 #Also experimentally determined by google calc using (ranking diff)^X until satisfactory

def get_arbitrary_team_rankings():
    rankings = []
    rankings.append("Germany")
    rankings.append("Belgium")
    rankings.append("Argentina")
    rankings.append("Brazil")
    rankings.append("Spain")
    rankings.append("France")
    rankings.append("Portugal")
    rankings.append("Poland")
    rankings.append("Uruguay")    
    rankings.append("Mexico")
    rankings.append("Columbia")
    rankings.append("England")
    rankings.append("Croatia")
    rankings.append("Costa Rica")
    rankings.append("Senegal")
    rankings.append("Serbia")
    rankings.append("Egypt")
    rankings.append("Nigeria")
    rankings.append("Denmark")
    rankings.append("Switzerland")
    rankings.append("Japan")
    rankings.append("Iceland")
    rankings.append("Russia")
    rankings.append("Sweden")
    rankings.append("Peru")
    rankings.append("South Korea")
    rankings.append("Panama")
    rankings.append("Australia")
    rankings.append("Iran")
    rankings.append("Tunisia")
    rankings.append("Morocco")
    rankings.append("Saudi Arabia")
...
def calculate_importance(team_a, team_b):
    comp_teams = get_matchup_disparity(team_a, team_b)
    return ((33 - comp_teams[1]) + (33 - comp_teams[2])) / 2
...
print([k for (k, v) in sorted(matches.items(), key = itemgetter(1), reverse = True)])